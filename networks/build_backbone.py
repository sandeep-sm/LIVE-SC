...
class RGBSingleHead(nn.Module):
    ...
    def forward(self, x, mode=0):
        ...
        feat = self.encoder(x)
        # feat1 = self.encoder.maxpool(self.encoder.relu(self.encoder.bn1(self.encoder.conv1(x))))
        # feat2 = self.encoder.layer1(feat1)
        ...
        if frame_batch == 2:    
            feat = feat.reshape(x_shape[0],x_shape[1],feat.shape[1])
